...
def collect_all(
    ...
    def collect_one(rcsb_id: str) -> tuple[str, bool, str]:
        try:
            collector = TubulinETLCollector(rcsb_id)
    ...
    console.print(f"Collecting [bold yellow]{len(to_process)}[/bold yellow] structures with {workers} workers.")

    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {executor.submit(collect_one, rid): rid for rid in to_process}
        for i, future in enumerate(as_completed(futures), 1):
            rcsb_id, success, error_msg = future.result()
            if success:
                console.print(f"[{i}/{len(to_process)}] [green]{rcsb_id}[/green]")
                success_count += 1
            else:
                console.print(f"[{i}/{len(to_process)}] [red]{rcsb_id}: {error_msg}[/red]")
                fail_count += 1
                failed_ids.append(rcsb_id)

    console.print(f"\n[bold]Done:[/bold] {success_count} succeeded, {fail_count} failed.")
    if failed_ids:
        console.print(f"[red]Failed IDs:[/red] {', '.join(failed_ids)}")
