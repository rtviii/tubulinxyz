...
class ProcessedChain:
    pdb_id: str
    chain_id: str  # Can be Chain ID or Entity ID
    tubulin_class: str
    sequence: str
...
class TubulinStructureParser:
    ...
    def get_observed_data(
        self, file_path: Path, chain_id: str
    ) -> Tuple[str, List[int]]:
        try:
            ...
            for residue in chain:
                ...
                # Filter out HETATMs that are not Modified Residues
...
class TubulinIngestor:
    ...
    def process_sequence(
        self, sequence: str, identifier: str, family: str
    ) -> ProcessedChain:
        """
        Process a raw sequence string (Entity) without downloading PDB files.
        Generates implicit auth_ids (1..N).
        """
        # 1. Align
        aln_master, aln_target = self.mapper.align_sequence(identifier, sequence)

        # 2. Generate implicit auth_ids for the Entity
        auth_ids = list(range(1, len(sequence) + 1))

        # 3. Detect Mutations (Replicating exact logic from original process_chain)
        ...
        target_res_index = 0  # Index into sequence / auth_ids
    ...
    def process_chain(self, pdb_id: str, chain_id: str, t_class: str) -> ProcessedChain:
        """
        Legacy/Physical Process: Downloads CIF, extracts sequence + author IDs.
        """
        # 1. Fetch & Extract
        ...
        # 3. Detect Mutations (Original Logic)
        ma_to_auth = [-1] * self.ref_len
        observed_to_ma = []
        mutations = []
        ...
        for m_char, t_char in zip(aln_master, aln_target):
            ...
            # Match
                ...
                if master_index < self.ref_len:
                    ma_to_auth[master_index] = current_auth_id
                    observed_to_ma.append(master_index + 1)
